...
export default function MasterEkspedisiPage() {
  ...
  const [formData, setFormData] = useState({ nama_ekspedisi: '', is_active: true });
  ...
  const fetchData = async () => {
    setLoading(true);
    try {
      const res = await api.get('/master/ekspedisi');
      if (res.data.success) {
        setData(res.data.data);
      }
    } catch (error) {
      toast.error('Gagal mengambil data ekspedisi');
    } finally {
      setLoading(false);
    }
  };
  ...
  const filteredData = data.filter(item => 
    item.nama_ekspedisi.toLowerCase().includes(search.toLowerCase())
  );
  ...
  if (loading) {
    return <LoadingSpinner text="Memuat Data Ekspedisi..." fullScreen />;
  }

  const openTambahModal = () => {
    setModalMode('tambah');
    setFormData({ nama_ekspedisi: '', is_active: true });
    setEditId(null);
    setIsModalOpen(true);
  };
  ...
  const openEditModal = (item: any) => {
    setModalMode('edit');
    setFormData({ nama_ekspedisi: item.nama_ekspedisi, is_active: item.is_active });
    setEditId(item.id);
    setIsModalOpen(true);
  };
  ...
  const confirmDelete = (item: any) => {
    setConfirmModal({
      isOpen: true,
      id: item.id,
      nama: item.nama_ekspedisi
    });
  };

  const handleSave = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!formData.nama_ekspedisi.trim()) return toast.error('Nama ekspedisi wajib diisi');

    try {
      if (modalMode === 'tambah') {
        await api.post('/master/ekspedisi', formData);
        toast.success('Ekspedisi berhasil ditambahkan');
      } else {
        await api.put(`/master/ekspedisi/${editId}`, formData);
        toast.success('Ekspedisi berhasil diperbarui');
      }
      setIsModalOpen(false);
      fetchData();
    } catch (error: any) {
      toast.error(error.response?.data?.message || 'Gagal menyimpan ekspedisi');
    }
  };
  ...
  const handleDelete = async () => {
    ...
    try {
      await api.delete(`/master/ekspedisi/${confirmModal.id}`);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto">
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-800">Master Ekspedisi</h1>
          <p className="text-sm text-slate-500">Kelola daftar layanan ekspedisi kurir.</p>
        </div>
      ...
      <Card>
        <CardContent className="p-0">
            ...
            <table className="w-full text-sm text-left">
              <thead className="bg-slate-50 text-slate-500 font-medium border-b uppercase text-xs">
                <tr>
                  ...
                  <th className="px-6 py-4">Nama Ekspedisi</th>
              ...
              <tbody className="divide-y divide-slate-100">
                {filteredData.length === 0 ? (
                  <tr><td colSpan={4} className="text-center py-10 text-slate-500">Tidak ada data ekspedisi</td></tr>
                ) : (
                  currentData.map((item, index) => (
                    <tr key={item.id} className="hover:bg-slate-50 transition-colors">
                      <td className="px-6 py-4 text-center font-medium text-slate-600">{(currentPage - 1) * itemsPerPage + index + 1}</td>
                      <td className="px-6 py-4 font-semibold text-slate-800">{item.nama_ekspedisi}</td>
          ...
          <div className="p-4 border-t flex items-center justify-between text-sm text-slate-500">
             <span>Menampilkan <strong>{currentData.length}</strong> dari <strong>{filteredData.length}</strong> ekspedisi</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
            <div className="p-4 border-b flex justify-between items-center bg-slate-50">
              <h3 className="font-semibold text-lg text-slate-800">{modalMode === 'tambah' ? 'Tambah Ekspedisi' : 'Edit Ekspedisi'}</h3>
            ...
            <form onSubmit={handleSave}>
              <div className="p-4 space-y-4">
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Nama Ekspedisi</label>
                  <Input 
                    placeholder="Contoh: JNE, J&T, dll" 
                    value={formData.nama_ekspedisi}
                    onChange={(e: React.ChangeEvent<HTMLInputElement>) => setFormData({...formData, nama_ekspedisi: e.target.value})}
                    required
                    autoFocus
                  />
                </div>
        ...
        <div className="fixed inset-0 z-[60] flex items-center justify-center bg-black/50 p-4 backdrop-blur-sm">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-sm overflow-hidden flex flex-col">
             ...
             <div className="p-4 text-slate-600 space-y-2">
                ...
                <p className="text-xs text-red-500 mt-2">Tindakan ini tidak dapat dibatalkan jika ekspedisi belum digunakan pada paket manapun.</p>
             </div>
             <div className="p-4 border-t bg-slate-50 flex justify-end gap-3">
